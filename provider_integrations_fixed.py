...
CPAGRIP_OFFER_LIMIT = max(1, int(os.getenv("CPAGRIP_OFFER_LIMIT", "3") or "3"))
...
def sync_cpagrip_offers(user_id: int) -> int:
    ...
    params = {"limit": CPAGRIP_OFFER_LIMIT}
    if "?" not in url:
...
def get_provider_offers(user_id: int, providers: Optional[Iterable[str]] = None):
    ...
    # Enforce the configured display limit here as well. This is important
    # because old/stale offers remain cached in MongoDB even after a feed
    # returns fewer offers.
    result = [dict(x) for x in docs if str(x.get("offer_id")) not in disabled]
    return result[:CPAGRIP_OFFER_LIMIT]
